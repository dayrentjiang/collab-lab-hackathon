import { 
  ...
  export const MOCK_APPLICATIONS: Application[] = [
    ...
    },
    {
      ...
      project_id: 1,
      user_id: 5,
      application_msg: "I'd like to join this project to contribute my frontend development skills. I've worked with React and Next.js extensively and am passionate about creating accessible interfaces.",
...
export async function getUserWithRelations(userId: number): Promise<UserWithRelations | null> {
  // Simulate API delay
  ...
  // Get user skills
  const userSkillIds = MOCK_USER_SKILLS
    .filter(us => us.user_id === userId)
    .map(us => us.skill_id);
  
  const skills = MOCK_SKILLS.filter(skill => userSkillIds.includes(skill.skill_id));
  
  // Get user projects
  const userProjects = MOCK_USER_PROJECTS.filter(up => up.user_id === userId);
  
  const projectIds = userProjects.map(up => up.project_id);
  const projects = MOCK_PROJECTS.filter(project => projectIds.includes(project.project_id));
...
export async function getProjectWithRelations(projectId: number): Promise<ProjectWithRelations | null> {
  ...
  // Get creator
  const creator = MOCK_USERS.find(u => u.user_id === project.project_creator_id);
  
  // Get required skills
  const projectSkillIds = MOCK_PROJECT_SKILLS
    .filter(ps => ps.project_id === projectId)
    .map(ps => ps.skill_id);
  
  const required_skills = MOCK_SKILLS.filter(skill => projectSkillIds.includes(skill.skill_id));
  
  // Get members
  const userProjects = MOCK_USER_PROJECTS.filter(up => up.project_id === projectId);
  const memberIds = userProjects.map(up => up.user_id);
  const members = MOCK_USERS.filter(user => memberIds.includes(user.user_id));
  
  // Get applications
  const applications = MOCK_APPLICATIONS.filter(app => app.project_id === projectId);
...
export async function getApplicationWithRelations(applicationId: number): Promise<ApplicationWithRelations | null> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 200));
  
  const application = MOCK_APPLICATIONS.find(a => a.application_id === applicationId);
  if (!application) return null;
  
  const user = MOCK_USERS.find(u => u.user_id === application.user_id);
  const project = MOCK_PROJECTS.find(p => p.project_id === application.project_id);
...
export async function getMessageWithRelations(messageId: number): Promise<MessageWithRelations | null> {
  // Simulate API delay
...
export async function getRecommendedProjects(category?: string): Promise<ProjectWithRelations[]> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 300));
...
export async function getUserApplicationsWithRelations(userId: number) {
  ...
  const applications = MOCK_APPLICATIONS.filter(app => app.user_id === userId);
