...
export default async function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const statusColor = statusColors[project.project_status] || statusColors.completed;
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColor}`}>
                  {project.project_status.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.skills && project.skills.map((skill, index) => (
                  <span 
                    key={index}
                    className={`px-3 py-1 rounded-full text-xs font-medium ${getSkillCategoryColor(skill.skill_category)}`}
                  >
                    {skill.skill_name}
                  </span>
                ))}
            ...
            {project.project_creator && (
              <div className="flex flex-col items-center bg-gray-50 p-4 rounded-lg border border-gray-100 min-w-48">
                <div className="w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center text-white text-xl font-medium mb-2">
                  {project.project_creator.user_name?.substring(0, 2).toUpperCase() || 'UN'}
                </div>
                <div className="text-center">
                  <p className="font-medium text-gray-800">{project.project_creator.user_name}</p>
                  <p className="text-sm text-gray-500">Project Creator</p>
                  {project.project_creator.user_bio && (
                    <p className="text-xs text-gray-600 mt-2 line-clamp-3">{project.project_creator.user_bio}</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1">
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-4">
                <p className="text-gray-700 mb-4">This project needs {project.project_vacancy} team members with the following skills:</p>
                ...
                {project.project_status === 'in_progress' && (
                  <div className="bg-yellow-50 p-3 rounded-md text-sm text-yellow-800">
                    This project is already in progress. Applications are currently closed.
                  </div>
                )}
            ...
            <div className="md:col-span-2">
              <h2 className="text-lg font-semibold text-gray-800 mb-4">About This Project</h2>
              ...
              <div className="prose prose-blue max-w-none">
                ...
                {project.project_timeline && (
                  <div className="mt-6">
                    <h3 className="font-medium text-gray-800 mb-2">Project Timeline</h3>
                    <p className="text-gray-600">{project.project_timeline}</p>
                  </div>
                )}
                
                <div className="mt-6">
                  <h3 className="font-medium text-gray-800 mb-2">Looking For</h3>
                  <p className="text-gray-600">
                    This project is looking for {project.project_vacancy} team members 
                    {project.skills && project.skills.length > 0 
                      ? ` with skills in ${project.skills.map(s => s.skill_name).join(', ')}` 
                      : ''}.
                  </p>
                </div>
                
                {project.project_creator && project.project_creator.user_bio && (
                  <div className="mt-6">
                    <h3 className="font-medium text-gray-800 mb-2">About the Creator</h3>
                    <p className="text-gray-600">{project.project_creator.user_bio}</p>
                  </div>
                )}
              </div>
              ...
              <div className="mt-8 p-4 bg-blue-50 rounded-lg">
                <h3 className="font-medium text-blue-800 mb-2">Interested in this project?</h3>
                <p className="text-blue-600 mb-4">Apply to join the team or message the project creator for more information.</p>
